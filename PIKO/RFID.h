...
struct RFID{//modelise une carte RFID
	HANDLE h;
	BYTE id_read[BUFSIZE];//id qu'on lit (a cause de l'encryption)
	BYTE id_true[BUFSIZE];//id ecrit sur le tag rfid en vrai
	...
	bool isConnected;
	bool NoTag;//on ne lit aucun tag (evite de reinitialiser le buffer a chaque fois)
